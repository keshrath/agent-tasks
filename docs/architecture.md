...
│   ├── rest.ts           # 18 REST endpoints + static file serving
...
SQLite with WAL mode at `~/.agent-tasks/agent-tasks.db`. Schema is versioned with automatic migrations (currently V3).
...
| Transport: MCP        | ~35   | All 33 tools, input validation, error formatting             |
| Transport: REST       | ~30   | All 18 endpoints, query params, error codes                  |
