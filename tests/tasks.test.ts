...
describe('claiming without advancing the stage', () => {
  it('claims a task without advancing the stage', () => {
    const task = ctx.tasks.create({ title: 'Assign me' }, 'agent-1');
    expect(task.stage).toBe('backlog');
    const assigned = ctx.tasks.claim(task.id, 'agent-2', false);
  ...
  it('respects custom pipelines (stays at the first stage)', () => {
    ...
    const assigned = ctx.tasks.claim(wave.id, 'head-influencer', false);
    expect(assigned.stage).toBe('I-0-pull');
    expect(assigned.assigned_to).toBe('head-influencer');
    expect(assigned.status).toBe('in_progress');
  });

  it('rejects claiming a non-pending task without advance', () => {
    const task = ctx.tasks.create({ title: 'Already claimed' }, 'agent-1');
    ctx.tasks.claim(task.id, 'agent-1');
    expect(() => ctx.tasks.claim(task.id, 'agent-2', false)).toThrow('not pending');
  });
  ...
  it('emits task:claimed event (same channel as claim)', () => {
    ...
    ctx.tasks.claim(task.id, 'agent-2', false);
  ...
  it('respects min_confidence_for_claim gate (parity with claim)', () => {
    ...
    expect(() => ctx.tasks.claim(vague.id, 'agent-2', false)).toThrow('confidence');
  });
});
